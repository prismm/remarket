...
router.post('/login', (req, res, next) => {
    User.scope('unsanitized').findOne({
        ...
        .then(user => {
            ...
            } else { // this will attach the user to our passport, which will save the user in the session store
                req.login(user, err => {
                    if (err) next(err);
...
router.post('/signup', (req, res, next) => {
    User.create(req.body) //should check to see if user already exists -- if email is not unique this will fail quietly
        .then(user => {
            req.login(user, err => {
                if (err) next(err);
                else res.json(user.sanitize()); //added sanitize
            });
        })
        .catch(next);
})
...
router.get('/me', (req, res, next) => {
    res.json(req.user.sanitize()); //added sanitize
});
